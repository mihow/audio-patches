// The same 16 note polyphonic midi player using 
// a custom / arbritrary waveform from Adventure Kid
// http://www.adventurekid.se/AKRTfiles/AKWF/view/waveforms_index.html
//
// Converted to 256+1 samples for Teensy by DataNoiseTV
// https://github.com/DatanoiseTV/AKWF_WaveForms
//
// Thanks Nantonos for posting these!
// http://forum.pjrc.com/threads/24793-Audio-Library?p=54197&viewfull=1#post54197
// 
//
...
// 16 different waveforms sampled from an organ. 
// Selected arbitrarily for each voice of the william tell overture
int16_t const* wave_type[16] = {
  AKWF_eorgan_0001,
  AKWF_eorgan_0002,
  AKWF_eorgan_0003,
  AKWF_eorgan_0004,
  ...
  */
  ...
  if(opcode == CMD_PLAYNOTE) {
    ...
    waves[chan]->begin(AMPLITUDE * velocity2amplitude[velocity-1],
                       ...
                       WAVEFORM_ARBITRARY);
    waves[chan]->arbitraryWaveform(wave_type[chan], MAX_FREQ);
    envs[chan]->noteOn();
    AudioInterrupts();
    return;
  }
